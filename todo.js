...
addItem=()=>{
        ...
        if(item && timeleft>0){
            ...
            for (var i = 0; i < tasks.length; i++){
                document.getElementById("todolist").innerHTML += ("<div id=" + i + " role='alert'><div>" + tasks[i][0] + "<span type='button' class='close' data-dismiss='alert' aria-label='Close'><span aria-hidden= 'true'>&times;</span></span></div><div>" + tasks[i][1] + " day(s)</div></div>");
                if (tasks[i][1] > 6)
                {
